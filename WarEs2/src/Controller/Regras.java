...
public class Regras {
    ...
    public boolean VerificaTrocaObrigatoria(Jogador j) {
        if (j.getCartasTerritorio().size() >= 5) {
            return true;
        } else {
            return false;
        }
    ...
    public void atualizaAposConquista(Jogador vencedor, Jogador perdedor, Estado conquistado, Estado atacante, int qtdDeslocamento) {
        ...
        vencedor.getEstados().add(conquistado.getIdEstado());
        perdedor.getEstados().remove(conquistado.getIdEstado());
    ...
    public void batalha(Estado atacante, Estado defensor, int numExercitos, int[] dadosAtacante, int[] dadosDefensor) {
        ...
        defensor.setNumeroExercitos(defensor.getNumeroExercitos() - retorno[1]);
        atacante.setNumeroExercitos(atacante.getNumeroExercitos() - retorno[0]);
